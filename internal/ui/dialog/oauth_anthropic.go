...
func (m *OAuthAnthropic) headerView() string {
	...
	return common.DialogTitle(t, t.Dialog.Title.Render(title), m.width-headerOffset, t.Dialog.TitleGradFromColor, t.Dialog.TitleGradToColor)
}

func (m *OAuthAnthropic) tabBar() string {
	t := m.com.Styles
	activeTab := lipgloss.NewStyle().
		Background(t.Dialog.TitleGradFromColor).
		Foreground(t.Dialog.PrimaryText.GetForeground()).
		Padding(0, 1)
	inactiveTab := lipgloss.NewStyle().
		Foreground(t.Dialog.SecondaryText.GetForeground()).
		Padding(0, 1)
	if !m.useAPIKey {
...
func (m *OAuthAnthropic) bodyView() string {
	t := m.com.Styles
	muted := lipgloss.NewStyle().Foreground(t.Dialog.SecondaryText.GetForeground())
	green := lipgloss.NewStyle().Foreground(t.Dialog.OAuth.Success.GetForeground())
	errStyle := lipgloss.NewStyle().Foreground(t.Dialog.OAuth.ErrorText.GetForeground())
	white := lipgloss.NewStyle().Foreground(t.Dialog.PrimaryText.GetForeground())
