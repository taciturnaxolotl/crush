...
func (c *coordinator) buildProvider(providerCfg config.ProviderConfig, model config.SelectedModel, isSubAgent bool) (fantasy.Provider, error) {
	...
	if providerCfg.Type == anthropic.Name {
		...
		// Add OAuth beta headers if using OAuth token
		if providerCfg.OAuthToken != nil {
			betaFeatures = append(betaFeatures, "oauth-2025-04-20", "claude-code-20250219")
		}

		// Add thinking beta header if using thinking mode
		...
		if len(betaFeatures) > 0 {
			if v, ok := headers["anthropic-beta"]; ok && v != "" {
				betaFeatures = append([]string{v}, betaFeatures...)
			}
